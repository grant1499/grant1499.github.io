...
在实现形式上，埃氏筛是直接用素数遍乘自然数删去自身倍数；线性筛是先建立一个素数数列，然后用每个自
...
如何保证一个数字只被筛掉一次:这个数字只被它的最小质因数筛掉，当它的其他质因数想要筛掉它时，将无
...
eg.当i = 4时，素数数组已经有2和3，`4*2`以p[0]=2为最小质因数，要筛掉。因为4 % 2 == 0，所以break。但是`4*3`不是以3而是2为最小质因数，它应该被`2*6`筛掉。
