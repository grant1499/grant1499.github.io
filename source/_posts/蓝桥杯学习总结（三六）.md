...
本题是acwing.1222. 密码脱落的例题，也是考察**区间DP**。（与POJ2955类似）

与密码脱落这题不同的地方在于A、B乘起来的结果是GBE，类似于密码脱落和石子合并两题的结合。

若括号序列是回文串，那么一定是合法的。但是该题新加了一个条件即()[]这样并列的括号也是合法的。

这里的定义2中的GBE也就是一个**回文序列**（如`[()()]`），定义2中的GBE是一个**并列括号序列**（如`()[]`）。

注意一下：定义2中的GBE和回文序列还是有一点区别的，单个符号可以是回文序列，但不满足GBE。

算法1：y式DP分析法。

![image-20210628210930813](https://gitee.com/grant1499/blog-pic/raw/master/img/202110231911262.png)
...
![image-20210628220235949](https://gitee.com/grant1499/blog-pic/raw/master/img/202110231911282.png)
