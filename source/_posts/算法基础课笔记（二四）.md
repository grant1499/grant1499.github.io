...
![image-20210907145037001](https://gitee.com/grant1499/blog-pic/raw/master/img/202110231948262.png)
...
关于滚动数组如何判断应该正序遍历还是逆序遍历的解释：

![image-20211006213601498](https://gitee.com/grant1499/blog-pic/raw/master/img/202110231948894.png)

![image-20211006213659669](https://gitee.com/grant1499/blog-pic/raw/master/img/202110231948963.png)
...
看算法训练营的表格图解模拟一下就很容易理解了。(注意：算法训练营**代码有问题已经勘误**！！！)

特别注意：`d[]`数组并不能真实记录最长上升子序列，只是辅助数组，这种做法只能求出最长上升子序列长度`len`。正确理解加粗句子就能看懂。最终的`d[]`数组是严格单调递增的，因为长度为`i`的上升子序列末尾的最小数字是逐渐变大的。

样例：`1 7 3 5 8 4`。最终得到的d数组：`1 3 4 8`，显然不是最长上升子序列，但长度没错。

更新辅助数组`d[]`有点贪心的意思，每次把一个数加入数组`d[]`，会考虑尽量使得这个数在“合适”的位置，通过二分找到这个“合适”的位置，保证上升子序列足够长。
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 2;i <= n;i ++){
			if (d[len] == a[i]) continue;
				...
				int l = 1,r = len; // lower_bound找到第一个大于等于a[i]的位置
				while (l < r){
					int mid  = l + r >> 1;
