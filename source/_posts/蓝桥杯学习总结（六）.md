...
![image-20210417113943246](https://gitee.com/grant1499/blog-pic/raw/master/img/202110231903168.png)

题目具有二段性，假设有E满足条件，比E小的数都不满足，比E大的数都满足。这点可以简单论证一下。

![image-20210417123437384](https://gitee.com/grant1499/blog-pic/raw/master/img/202110231903208.png)

注意一个坑：如果E接近h的最大值时，`E*2`容易爆int（记住：int范围约是2*10$^9$.），其实我们只要判断`E>=max(h)`，满足的话后面E肯定是递增的，就不用再判断是否大于0了。证明如下：

![image-20210417124151973](https://gitee.com/grant1499/blog-pic/raw/master/img/202110231903232.png)
...
bool check(int m){
    for (int i = 1;i <= n;i ++){
        m = 2*m - num[i];
        if (m >= 1e5) return true; // E >= max h，提前退出
        if (m < 0) return false;
    }
    return true; // 这里别忘了
}
...
设跳到第 k 座建筑后的能量为`a[k]`，有递推关系：`a[k] = 2*a[k-1] - h[k], k >= 1`

必须保证`a[k] >= 0, 0 <= k <= n`，所以有：

`a[0] >= h[1] / 2, a[1] >= h[2] / 2, ... , a[n-1] >= h[n] / 2`

推导出：`a[0] >= h[1] / 2 + h[2] / 2^2 + ... + h[n] / 2^n`
...
![image-20210417155749683](https://gitee.com/grant1499/blog-pic/raw/master/img/202110231903257.png)
